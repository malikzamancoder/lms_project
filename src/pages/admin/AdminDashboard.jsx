...
const AdminDashboard = () => {
  const [counts, setCounts] = useState({ teachers: 0, students: 0, quizzes: 0, assignments: 0 });

  useEffect(() => {
    const unsubs = [
      onSnapshot(query(collection(db, "users"), where("role", "==", "teacher")), (s) =>
        setCounts((c) => ({ ...c, teachers: s.size }))
      ),
  ...
  return (
    <div>
      <h2 className="text-white text-xl font-semibold mb-6">Admin Dashboard</h2>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Teachers" value={counts.teachers} icon="👨‍🏫" />
        <StatCard label="Students" value={counts.students} icon="🎓" />
        <StatCard label="Quizzes" value={counts.quizzes} icon="❓" />
