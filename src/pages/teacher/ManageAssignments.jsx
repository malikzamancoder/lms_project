...
const ManageAssignments = () => {
  ...
  const toggleEnabled = async (assignmentId, current) => {
    await updateDoc(doc(db, "assignments", assignmentId), { enabled: !current });
  };
  ...
  return (
    <div>
      <h2 className="text-white text-xl font-semibold mb-6">Manage Assignments</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {assignments.length === 0 && (
          <p className="text-slate-500 text-sm col-span-2 text-center py-6">No assignments created yet</p>
        )}
      </div>
    </div>
  );
